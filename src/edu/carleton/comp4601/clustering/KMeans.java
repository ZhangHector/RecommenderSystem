...
public class KMeans {
	...
	public KMeans(int numClusters, List<edu.carleton.comp4601.model.User> userList) {
		changed = true;
		no_users = userList.size();
		no_features = NUM_FEATURES;
		users = new User[no_users];
		no_clusters = numClusters;
		...
		for(int i = 0; i < userList.size(); i++) {
			edu.carleton.comp4601.model.User userToTransform = userList.get(i);
			String name = userToTransform.getName();
			double commedy = userToTransform.getComedy();
			double drama = userToTransform.getDrama();
			double family = userToTransform.getFamily();
			double horror = userToTransform.getHorror();
			double other = userToTransform.getOther();
			users[i] = new User(name, no_features, no_clusters);
			users[i].features[0] = commedy;
			users[i].features[1] = drama;
			users[i].features[2] = family;
			users[i].features[3] = horror;
			users[i].features[4] = other;
		}		
...
//			System.out.println("x------------------x");
		...
		System.out.println("Updating in Local Database...");
//		for(int i = 0; i < clusters.length; i++) 
//			System.out.println(newCentroids[i]);
		for(int i = 0; i < clusters.length; i++)
			System.out.println("Last Cluster: " + clusters[i].size());
		StringBuilder sb = new StringBuilder();
		sb.append(CSV.CENTER_USER_HEADING);
		for (User user : newCentroids){
			edu.carleton.comp4601.model.Cluster centerCluster = new edu.carleton.comp4601.model.Cluster();
			centerCluster.setCluster(Integer.parseInt(user.name.split("_")[1]));
			centerCluster.setComedyClass((int) Math.ceil(user.features[0]));
			centerCluster.setDramaClass((int) Math.ceil(user.features[1]));
			centerCluster.setFamilyClass((int) Math.ceil(user.features[2]));
			centerCluster.setHorrorClass((int) Math.ceil(user.features[3]));
			centerCluster.setOtherClass((int) Math.ceil(user.features[4]));
			sb.append(centerCluster.convertToCSV());
		}
		CSV.writeCenterUser(sb.toString());
//		printClusters(clusters);
	...
	public double getDistance(){
		...
		for(int i = 0; i < clusters.length; i++) {
			...
			for (User user : cluster)
				result += distance(user, newCentroids[i]);
		}
		return result;
	}
	...
	/**
	 * Generates a random point in N-dimensions between the min and max bounds.
	 */
	...
	private User randomPoint(User min, User max) {
		...
		for(int f = 0; f < numFeatures; f++) {
			...
			if (Double.compare(currentMin, currentMax) == 0)
				currentMax += 0.0000001;
			double random =  ThreadLocalRandom.current().nextDouble(currentMin, currentMax);
	...
	/**
	 *  Computes the centroid of the given cluster by taking the average of all users.
	 */
...
//	}
	...
	public List<edu.carleton.comp4601.model.UserCluster> convertUsers(List<User>[] clusters) {
		List<edu.carleton.comp4601.model.UserCluster> allClusteredUsers = new ArrayList<edu.carleton.comp4601.model.UserCluster>();
		for(int i = 0; i < clusters.length; i++) {
			List<User> cluster = clusters[i];
			List<edu.carleton.comp4601.model.UserCluster> clusteredUsers = userAdapter(cluster, i);
	...
	public List<edu.carleton.comp4601.model.UserCluster> userAdapter(List<User> kMeansUsers, int clusterNumber) {
		List<edu.carleton.comp4601.model.UserCluster> clusterUsers = new ArrayList<edu.carleton.comp4601.model.UserCluster>();
		for(User kMeansUser: kMeansUsers) {
			edu.carleton.comp4601.model.UserCluster userCluster = new edu.carleton.comp4601.model.UserCluster();
